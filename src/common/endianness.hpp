...
// Sanity check
#if !defined(ADL2VGM_LITTLE_ENDIAN) && !defined(ADL2VGM_BIG_ENDIAN)
...
#if !defined(ADL2VGM_NEED_ALIGNMENT)
...
// use software fallback by loading each byte explicitely
	...
	#if defined(ADL2VGM_LITTLE_ENDIAN)
	...
	#elif defined(ADL2VGM_BIG_ENDIAN)
...
// if the unaligned load and the byteswap take alot instructions its better to directly read and invert
	#if defined(ADL2VGM_NEED_ALIGNMENT) && !defined(__mips__)
	...
	#endif // if defined(ADL2VGM_NEED_ALIGNMENT)
...
#endif // if defined(ADL2VGM_LITTLE_ENDIAN)
